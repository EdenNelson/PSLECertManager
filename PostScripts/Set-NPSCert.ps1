<#
.SYNOPSIS
    Deploys a Let's Encrypt certificate to Network Policy Server (NPS).

.DESCRIPTION
    This script is a post-action script invoked after successful certificate creation or renewal by Update-Certificate.ps1.
    It handles both staging and production certificate deployments to Network Policy Server:
    
    - STAGING mode: Validates that the certificate was created and exists in the certificate store.
      No changes are made to NPS services (staging certificates are untrusted by browsers).
    
    - PRODUCTION mode: Deploys the certificate to NPS by:
      1. Validating the certificate exists and is valid for NPS use
            2. Updating NPS certificate thumbprint registry configuration
      3. Restarting the NPS service to apply changes

        Configuration is self-discovered from Vars.psd1 under the 'Set-NPSCert' subobject.
        Optional keys:
        - ServiceName (default: IAS)
        - CertHashRegistryPath (default: HKLM:\SYSTEM\CurrentControlSet\Services\AuthSvc\Parameters)
...
.PARAMETER UseStaging
    ...
    When present, the script validates the certificate without making any NPS changes.
...
.NOTES
    ...
    Requirements:
    - Network Policy Server (NPS) role installed and configured
    - Administrator/SYSTEM privileges
    - Certificate must be in LocalMachine\My certificate store
    - Windows Server 2016 or later

.EXAMPLE
    .\Set-NPSCert.ps1 -LatestCertThumbprint "ABC123DEF456" -Verbose
    
    Deploys the certificate with thumbprint ABC123DEF456 to NPS in production mode.

.EXAMPLE
    .\Set-NPSCert.ps1 -LatestCertThumbprint "ABC123DEF456" -UseStaging -Verbose
    
    Validates the staging certificate with thumbprint ABC123DEF456 without deploying to NPS.
#>
[CmdletBinding()]
param (
...
begin {
    ...
    if (-not (Test-Path -Path $varsPath)) {
        throw "Vars.psd1 not found at $varsPath"
    }

    $scriptKey = $script:ScriptName.Replace('.ps1', '')
    $config = (Import-PowerShellDataFile -Path $varsPath)[$scriptKey]
    if (-not $config) {
        throw "No configuration subobject '$scriptKey' found in Vars.psd1"
    }

    $script:NpsServiceName = if ($config.ContainsKey('ServiceName') -and $config.ServiceName) { $config.ServiceName } else { 'IAS' }
    $script:NpsRegPath = if ($config.ContainsKey('CertHashRegistryPath') -and $config.CertHashRegistryPath) {
        $config.CertHashRegistryPath
    }
    else {
        'HKLM:\SYSTEM\CurrentControlSet\Services\AuthSvc\Parameters'
    }
    ...
    Write-Log "NPS Service Name: $script:NpsServiceName"
    Write-Log "NPS Cert Hash Registry Path: $script:NpsRegPath"
    ...
    if ($UseStaging) {
        ...
        Write-Verbose -Message "Certificate created successfully with thumbprint: $LatestCertThumbprint"
        Write-Verbose -Message "Staging mode detected; skipping deployment and validating cert exists."
        ...
        if ($cert) {
            ...
            Write-Verbose -Message "Certificate found in LocalMachine\My store"
            ...
            Write-Verbose -Message "Staging cert validated; no service changes made."
            Write-Verbose -Message "Staging certificate NOT deployed to NPS (staging certificates are not trusted)."
            ...
            throw "Certificate with thumbprint $LatestCertThumbprint not found in certificate store"
        }
    ...
    Write-Log "PRODUCTION MODE - Deploying certificate to NPS"
    Write-Verbose -Message "===== PRODUCTION MODE - Deploying certificate to NPS ====="
    Write-Verbose -Message "Certificate thumbprint: $LatestCertThumbprint"
    Write-Verbose -Message "Starting Network Policy Server certificate deployment."

    # Verify certificate exists in certificate store
    try {
        Write-Log "Validating certificate in LocalMachine\My store"
        $cert = Get-ChildItem -Path Cert:\LocalMachine\My | Where-Object -FilterScript { $_.Thumbprint -eq $LatestCertThumbprint }
        if (-not $cert) {
            Write-Log "Certificate with thumbprint $LatestCertThumbprint not found in certificate store" "ERROR"
            throw "Certificate with thumbprint $LatestCertThumbprint not found in certificate store"
        }
        Write-Log "Certificate validated: Subject=$($cert.Subject), NotAfter=$($cert.NotAfter)"
        Write-Verbose -Message "Certificate found and validated"
        Write-Verbose -Message "  Subject: $($cert.Subject)"
        Write-Verbose -Message "  NotAfter: $($cert.NotAfter)"
    }
    catch {
        Write-Log "Failed to validate certificate: $_" "ERROR"
        throw
    }

    # Configure NPS to use the new certificate
    try {
        Write-Log "Configuring NPS to use certificate with thumbprint $LatestCertThumbprint"
        Write-Verbose -Message "Updating NPS registry configuration with new certificate thumbprint"
        
        # Check if the registry path exists
        if (-not (Test-Path -Path $script:NpsRegPath)) {
            Write-Log "Warning: NPS registry path not found. Ensure NPS role is installed." "WARN"
            throw "NPS registry path not found: $script:NpsRegPath"
        }
        else {
            # Update the certificate thumbprint in NPS configuration
            Set-ItemProperty -Path $script:NpsRegPath -Name "CertHash" -Value $LatestCertThumbprint -ErrorAction Stop
            Write-Log "NPS certificate configuration updated successfully"
            Write-Verbose -Message "NPS certificate thumbprint configured in registry"
        }
    }
    catch {
        Write-Log "Failed to update NPS configuration: $_" "ERROR"
        throw
    }

    # Restart NPS service to apply changes
    try {
        Write-Log "Restarting Network Policy Service ($script:NpsServiceName)"
        Write-Verbose -Message "Stopping Network Policy Service..."
        Stop-Service -Name $script:NpsServiceName -Force -ErrorAction Stop

        Write-Verbose -Message "Waiting for service to stop completely..."
        Start-Sleep -Seconds 2

        Write-Verbose -Message "Starting Network Policy Service..."
        Start-Service -Name $script:NpsServiceName -ErrorAction Stop

        Write-Verbose -Message "Waiting for service to start..."
        Start-Sleep -Seconds 2

        Write-Log "Network Policy Service restarted successfully"
        Write-Verbose -Message "NPS service restart completed"
    }
    catch {
        Write-Log "Failed to restart Network Policy Service: $_" "ERROR"
        throw
    }

    # Verify service is running
    try {
        Write-Log "Verifying Network Policy Service is running"
        $serviceStatus = Get-Service -Name $script:NpsServiceName -ErrorAction Stop
        if ($serviceStatus.Status -eq "Running") {
            Write-Log "Network Policy Service is running successfully"
            Write-Verbose -Message "Network Policy Service status: $($serviceStatus.Status)"
        }
        else {
            Write-Log "Network Policy Service failed to start or stopped unexpectedly" "ERROR"
            throw "Network Policy Service failed to start or stopped unexpectedly"
        }
    }
    catch {
        Write-Log "Failed to verify Network Policy Service status: $_" "ERROR"
        throw
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwvSF8c+WrmoiZ/0ZJq4nq5Nj
# G32gghk5MIIGFDCCA/ygAwIBAgIQeiOu2lNplg+RyD5c9MfjPzANBgkqhkiG9w0B
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUJenCCVUKLSb9f60LUlPgH+Ks
# ITIwDQYJKoZIhvcNAQEBBQAEggEA0e1JL9gxafw60x88Cc3hcSTU9FQY4mbAwBrW
# Bk5cRyC1ZJ6u4R5/s2eP/rIesOvObNyQ/04VbVmDCouZA2Ssnp2sTXBeqcydqgeR
# /A8kxnQIgIa42j5LrEu58kIQSoO9h+lj7sdhtOJHgu0HVhLH6tt+6sYYvaxeMHwO
# AhcbcrFk0pH+PTsU+9VP5+UHrJpKFPO9azAuByBf9yxuhidfBznjlx3YbuY9UEP7
# HDurbE3gNxqU8mZWCki6yeKXpsnZxdlkrxQWhUySFSA17CqOnuU7r34jC1lQoRCi
# FvtcVTfizNtYaSsIMm24WjpNOsXGr44SfdmmNkEBHKUdMUwvcKGCAyMwggMfBgkq
...
# NDE3NDYwMlowPwYJKoZIhvcNAQkEMTIEMLutuqxjo777tsCWcRpTh6PbydBvsF81
# o1xGtvAGbyz5Cp8Afdw3k+PNVtS6py1tyTANBgkqhkiG9w0BAQEFAASCAgCZbe6c
# OnzGaJkbWfSyOpOfBoDrWyU+g22j+wNZE/9uAcx8BGEPgPKnjd69F3nG5YcQEHq0
# kbXNEnBBuH3vOdEwbqfoC4wz17lHmvijZISCTAkqSnD+r0vIZGh1ZxMfLVSahg17
# wiC6/HORfeeNvil1pbP6BOaumrQByJPKoI0ohTPpGH/JSrD8421cr51XXp+yGtrO
# 7T8O1LvNrjcTSU5Jd5jmjphWgcQJ6a3HQsZaE080QavYCyngdmNZPxhi3WuW59C+
# t1RJ+BjbrNgZzC+8vhwesUaBGj/F90NbXaS8+b/0F5Ga6QWPsT6tl77IYLYskhY7
# TqepV+ajzrE1kC5LKJnDb9Q/NhLocswjBgLA/YboDsSM620OW5BDlfLY00yCM36P
# NebMa79IJSlbvxRFTK5JJwpq3zLfwk9nwcyVUpgbAG5HXMKurx8eFPQZWEsXnlGn
# 0qn2nuo6VIdniX9jT0paHuo0fEozt2ld7BbAT4ZFoslVUYCRYGbgkvEj6HaIwXrh
# 8R7bfYdZoeX2Hsf2b7pK8hyc9K2FQfwNLQIybDQecyvf4g/GspJE/g7tpP5DT/Rd
# gLQRV73yHsPeDcecdDLf8HUScQGZdqfQwhq7I0f7lxN0ZlF1zaJZEV7gz7Khlm8I
# vdW+44eDegPbhk1/sIoFfVJY6sKdl1bYbJgLbQ==
